...
name = 'user'
for i in range(2, 51):
    obj = User(id=i, password=password, is_superuser=0, username=name + str(i), first_name='User' + str(i),
               last_name='Cruiser' + str(i), email=name + str(i) + '@gmail.com', is_staff=0, is_active=1,
...
for p in ServiceProvider.objects.all():
    for s in range(1, 101):
        obj = Service(id=id, service_provider_id=p.id, name="Servisio" + str(s),
