...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        messages = []
        for reservation in Reservation.objects.select_for_update().filter(user__isnull=False, service__isnull=False,
                                                                          date__lte=now.date(),
                                                                          emailsent=False).order_by('date', 'time'):
            if now >= datetime.datetime.combine(reservation.date, reservation.time):
                ...
                messages.append((_('Review your service reservation'),
                                 render_to_string('emails/comment.html', {'reservation': reservation, 'url': url}),
                                 None, [reservation.user.email]))
                # Set emailsent
