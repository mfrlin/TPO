...
def convert_to_gcal_event(reservation):
    ...
    start = add_timezone(datetime.datetime.combine(reservation.date, reservation.time),
...
def sync(service_provider):
    ...
    # New reservations
    counter_created = 0
    for reservation in service_provider.reservations.select_for_update().filter(gcalimported__isnull=True,
                                                                                isfromgcal=False):
        event = service.events().insert(calendarId=service_provider.gcal_id,
                                        body=convert_to_gcal_event(reservation)).execute()
    ...
    # Updates
    counter_updated = 0
    for reservation in service_provider.reservations.select_for_update().filter(gcalimported__lt=F('updated'),
                                                                                isfromgcal=False):
        service.events().update(calendarId=service_provider.gcal_id, eventId=reservation.gcalid,
                                body=convert_to_gcal_event(reservation)).execute()
        reservation.gcalimported = F('updated')
        reservation.save()
        counter_updated += 1
    ...
    for event in get_all_items(service.events().list, calendarId=service_provider.gcal_id, singleEvents=True, **extra):
        if event['status'] == 'cancelled':
            ...
            reservation, created = Reservation.objects.get_or_create(service_provider=service_provider,
                                                                     gcalid=event['id'], defaults=fields)
