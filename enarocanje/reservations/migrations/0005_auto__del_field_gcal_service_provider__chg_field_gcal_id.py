...
class Migration(SchemaMigration):
    def forwards(self, orm):
        # Deleting field 'GCal.service_provider'
        db.delete_column(u'reservations_gcal', 'service_provider_id')

        # Renaming column for 'GCal.id' to match new field type.
        db.rename_column(u'reservations_gcal', u'id', 'id_id')
        # Changing field 'GCal.id'
        db.alter_column(u'reservations_gcal', 'id_id',
                        self.gf('django.db.models.fields.related.ForeignKey')(to=orm['accountext.ServiceProvider'],
                                                                              primary_key=True))
        # Adding index on 'GCal', fields ['id']
        db.create_index(u'reservations_gcal', ['id_id'])

    def backwards(self, orm):
        # Removing index on 'GCal', fields ['id']
        db.delete_index(u'reservations_gcal', ['id_id'])

        # User chose to not deal with backwards NULL issues for 'GCal.service_provider'
        raise RuntimeError("Cannot reverse this migration. 'GCal.service_provider' and its values cannot be restored.")

        # Renaming column for 'GCal.id' to match new field type.
        db.rename_column(u'reservations_gcal', 'id_id', u'id')
        # Changing field 'GCal.id'
        db.alter_column(u'reservations_gcal', u'id', self.gf('django.db.models.fields.AutoField')(primary_key=True))
