$(document).ready(function () {
    ...
    var addGalleryImage = function (file) {
        var outer_box = $('<div class="col-lg-3 col-md-4 col-xs-6 real-gallery-image"></div>');
        var fancy_box = $('<a></a>');
    ...
    $(document).ready(function () {
        ...
        $('.image-upload-bundle').fileupload({
            ...
            progressall: function (e, data) {
                ...
                if (added_count > 0) {
                    $('.span12.well.well-small.white-bg.clearfix').show();
                ...
                $.each(data.result.files, function (index, file) {
                    if ('error' in file) {
                        ...
                        /* adding delete buttons if necessary */
                        if ($('.real-gallery-image').length == 1) {
                            var row = $('.row');
                            var btn1 = $('<button class="btn btn-danger" type="submit" name="action" \
                            value="delete" style="margin-right: 5px">' + gettext('Delete selected') + '</button>&nbsp;');
                            var btn2 = $('<button class="btn btn-danger" type="submit" name="action" \
                            value="delete_all">' + gettext('Delete all images') + '</button>');
                            btn2.insertAfter(row);
                            btn1.insertAfter(row);
                        }
                    }
                });
